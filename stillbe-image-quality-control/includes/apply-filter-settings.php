...
add_action( 'plugins_loaded', function() {
	...
	add_filter( 'stillbe_image_quality_default_list', function( $default ) use( $settings ) {
		...
		// 新しい形式の既定値が保存済み設定に存在しない場合も補完する。
		return array_merge( $default, $settings['quality'] );
	...
	// Preferred Image Editor (imagick | gd)
	add_filter( 'stillbe_image_quality_control_preferred_image_editor', function( $pref ) use( $settings ) {

		if( empty( $settings['preferred-image-editor'] ) ) {
			return $pref;
		}

		return ( 'gd' === $settings['preferred-image-editor'] ) ? 'gd' : 'imagick';
	...
	add_filter( 'stillbe_image_quality_control_enable_interlace_jpeg', function( $is_enable ) use( $toggle ) {
		...
		return ! empty( $toggle['enable-progressive-jpeg'] );
	...
	// Enable AVIF; true
	add_filter( 'stillbe_image_quality_control_enable_avif', function( $is_enable ) use( $toggle ) {

		if( ! isset( $toggle['enable-avif'] ) ) {
			return $is_enable;
		}

		return ! empty( $toggle['enable-avif'] );
