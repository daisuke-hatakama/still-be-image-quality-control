...
trait Setting_Common_Methods {


	// Check Month, date, hour, minute
	// $allow_avif_lossless: 品質テスト等で AVIF 100 (Imagick ロスレス) を許可する
	static public function chk_num_type( $value, $type = null, $allow_avif_lossless = false ) {
		...
		switch( $type ) {
			...
			break;
			case 'avif':
				// ImageMagick は quality >= 100 で可逆 AVIF になるため、品質テーブルは 99 まで
				// 品質テスト時のみ $allow_avif_lossless で 100 を許可する
				$min = 1;
				$max = $allow_avif_lossless ? 100 : 99;
			break;
			case 'png':
				$min = 1;
				$max = 9;
	...
	public function sanitize_setting( $input ) {
		...
		$big_threashold = $big_threashold ? min( 16383, absint( $big_threashold ) ) : $big_threashold;   // Max Size of WebP is 16383px
		...
		foreach( $add_sizes as $add_size) {
			...
			foreach( array( 'jpeg', 'png', 'webp', 'avif' ) as $type ) {
		...
		if( empty( $save['quality']['default_avif'] ) ) {
			$save['quality']['default_avif'] = $this->_default['default_avif'];
		}
		// Deleted @since 1.0.0
	...
	//	}
		...
		// Auto Optimize Target Level (SSIM)
		$ao_target = isset( $input['auto-optimize-target'] ) ? $input['auto-optimize-target'] : 'balance';
		$save['auto-optimize-target'] = in_array( $ao_target, array( 'efficiency', 'balance', 'quality' ), true ) ? $ao_target : 'balance';

		// Auto Optimize Concurrency (simultaneous jobs)
		$ao_concurrency = isset( $input['auto-optimize-concurrency'] ) ? absint( $input['auto-optimize-concurrency'] ) : STILLBE_IQ_AUTO_OPTIMIZE_CONCURRENCY;
		if( $ao_concurrency < 1 ) {
			$ao_concurrency = STILLBE_IQ_AUTO_OPTIMIZE_CONCURRENCY;
		}
		$save['auto-optimize-concurrency'] = $ao_concurrency;

		// Preferred Image Editor
		$pref_editor = isset( $input['preferred-image-editor'] ) ? $input['preferred-image-editor'] : STILLBE_IQ_PREFERRED_IMAGE_EDITOR;
		$save['preferred-image-editor'] = ( 'gd' === $pref_editor ) ? 'gd' : 'imagick';
		...
		// Auto optimize on Re-Compression (default: false)
		if( ! isset( $save['toggle']['enable-auto-optimize-on-recompress'] ) ) {
			$save['toggle']['enable-auto-optimize-on-recompress'] = false;
		}

		// Purge delivery WebP on Re-Compression (default: false = keep until auto-optimize)
		if( ! isset( $save['toggle']['enable-purge-delivery-webp-on-recompress'] ) ) {
			$save['toggle']['enable-purge-delivery-webp-on-recompress'] = false;
		}
		...
		// If WebP or AVIF is Enabled, Put ".htaccess" in the uploads Directory
		_stillbe_iqc_htaccess_webp(
			! empty( $save['toggle']['enable-webp'] ) || ! empty( $save['toggle']['enable-avif'] )
		);
