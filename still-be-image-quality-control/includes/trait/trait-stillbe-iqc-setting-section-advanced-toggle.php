...
trait Setting_Section_Advanced_Toggle {
	...
	protected function add_section_advanced_toggle() {
		...
		add_action( 'admin_init', function() {
			...
			add_settings_field(
				STILLBE_IQ_PREFIX. 'sf-delete-original-large',   // Field ID (Slug)
				esc_html__( 'Delete Original Large Images', 'still-be-image-quality-control' ),   // Field Label
				array( $this, 'render_toggle_options' ),   // Rederer
				STILLBE_IQ_PREFIX. 'setting-page',   // Rendering Page
				STILLBE_IQ_PREFIX. 'ss-advanced-toggle',   // Section
				array(   // Arguments for Renderer Function
					'field'       => 'delete-original-large',
					'description' => array(
						esc_html__( 'When a large image is resized, delete the original large image file (like photos taken directly from a camera).', 'still-be-image-quality-control' ). '<br>'.
						esc_html__( 'This helps save server storage space by removing unnecessarily large original files.', 'still-be-image-quality-control' ). '<br>',
						esc_html__( 'Note: Once deleted, the original large image cannot be recovered.', 'still-be-image-quality-control' ). '<br>',
						esc_html__( 'Note: The threshold for large images is 2560px by default and can be changed.', 'still-be-image-quality-control' ),
					),
					'default'     => STILLBE_IQ_ENABLE_DELETE_ORIGINAL_LARGE,
				)
			);
			...
			add_settings_field(
				...
				array(   // Arguments for Renderer Function
					...
					'description' => array(
						...
						Image_Editor_Imagick::class === $this->_editor ? '' :
					...
					'default'     => (
						Image_Editor_Imagick::class === $this->_editor
							? STILLBE_IQ_ENABLE_INDEX_COLOR_IMAGICK
							: STILLBE_IQ_ENABLE_INDEX_COLOR_GD
					),
				)
			);
			...
			// Enable AVIF
			add_settings_field(
				STILLBE_IQ_PREFIX. 'sf-enable-avif',   // Field ID (Slug)
				esc_html__( 'Enable Automatically Generating AVIF', 'still-be-image-quality-control' ),   // Field Label
				array( $this, 'render_toggle_options' ),   // Rederer
				STILLBE_IQ_PREFIX. 'setting-page',   // Rendering Page
				STILLBE_IQ_PREFIX. 'ss-advanced-toggle',   // Section
				array(   // Arguments for Renderer Function
					'field'       => 'enable-avif',
					'description' => array(
						esc_html__( 'Automatically generate AVIF so that it can be delivered instead of JPEG or PNG.', 'still-be-image-quality-control' ),
						esc_html__( 'If user&#39s browser supports AVIF format, AVIF will be delivered preferentially.', 'still-be-image-quality-control' ),
						'<small class="your-server-status">'. __( 'AVIF enablement on your server:', 'still-be-image-quality-control' ).
							(
								stillbe_iqc_is_avif_editor_available() ?
									'<em class="available">'. esc_html__( 'Available', 'still-be-image-quality-control' ) :
									'<em class="unavailable">'. esc_html__( 'Unavailable', 'still-be-image-quality-control' )
							).
						'</em></small>',
					),
					'default'     => STILLBE_IQ_ENABLE_AVIF,
				)
			);

			// Auto optimize on Re-Compression (default: disable)
			add_settings_field(
				STILLBE_IQ_PREFIX. 'sf-enable-auto-optimize-on-recompress',   // Field ID (Slug)
				esc_html__( 'Optimize after Re-Compression', 'still-be-image-quality-control' ),   // Field Label
				array( $this, 'render_toggle_options' ),   // Rederer
				STILLBE_IQ_PREFIX. 'setting-page',   // Rendering Page
				STILLBE_IQ_PREFIX. 'ss-advanced-toggle',   // Section
				array(   // Arguments for Renderer Function
					'field'       => 'enable-auto-optimize-on-recompress',
					'description' => array(
						esc_html__( 'Run SSIM automatic optimization after re-compression finishes.', 'still-be-image-quality-control' ),
						esc_html__( 'Re-compressing many images can take a long time. This option is disabled by default.', 'still-be-image-quality-control' ),
					),
					'default'     => false,
				)
			);

			// Enable Lossless Compression (Imagick or cwebp; WebP / AVIF)
			add_settings_field(
				STILLBE_IQ_PREFIX. 'sf-enable-webp-lossless',   // Field ID (Slug)
				__( 'Enable Lossless Compression', 'still-be-image-quality-control' ),   // Field Label
				array( $this, 'render_toggle_options' ),   // Rederer
				STILLBE_IQ_PREFIX. 'setting-page',   // Rendering Page
				STILLBE_IQ_PREFIX. 'ss-advanced-toggle',   // Section
				array(   // Arguments for Renderer Function
					...
					'description' => array(
						esc_html__( 'You can enable lossless compression when converting PNG to alternative formats such as WebP or AVIF.', 'still-be-image-quality-control' ),
						...
						esc_html__( 'Available when the Imagick extension is installed.', 'still-be-image-quality-control' ),
						'<small class="your-server-status">'. __( 'Lossless compression enablement on your server:', 'still-be-image-quality-control' ).
							(
								(
									( stillbe_iqc_is_extended() && stillbe_iqc_is_enabled_cwebp() )
									|| (
										class_exists( '\Imagick' )
										&& class_exists( Image_Editor_Imagick::class )
						...
						'</em><i>WebP</i> / '.
							(
								(
									...
									&& class_exists( Image_Editor_Imagick::class )
									&& Image_Editor_Imagick::test()
									&& Image_Editor_Imagick::supports_mime_type( 'image/avif' )
								) ?
									'<em class="available">'. esc_html__( 'Available', 'still-be-image-quality-control' ) :
									'<em class="unavailable">'. esc_html__( 'Unavailable', 'still-be-image-quality-control' )
							).
			...
			if( stillbe_iqc_is_extended() ) {
				...
				// Enable Near Lossless Compression (cwebp only)
				add_settings_field(
					STILLBE_IQ_PREFIX. 'sf-enable-webp-near-lossless',   // Field ID (Slug)
					__( 'Enable Near-Lossless Compression', 'still-be-image-quality-control' ),   // Field Label
					array( $this, 'render_toggle_options' ),   // Rederer
					STILLBE_IQ_PREFIX. 'setting-page',   // Rendering Page
					STILLBE_IQ_PREFIX. 'ss-advanced-toggle',   // Section
					array(   // Arguments for Renderer Function
