...
trait Setting_Section_Test {
	...
	public function render_sd_test() {
		...
		// Imagick があれば PNG 可逆 (WebP / AVIF) をテストできる
		if( class_exists( '\Imagick' ) ) {
			$options['stillbe_image_quality_control_enable_webp_lossless_for_png_gif'] = esc_html__( 'Enable Lossless Compression', 'still-be-image-quality-control' );
		}
		if( stillbe_iqc_is_extended() ) {
			...
			$options['stillbe_image_quality_control_enable_webp_near_lossless'] = esc_html__( 'Enable Near-Lossless Compression', 'still-be-image-quality-control' );
		}
		...
		if( stillbe_iqc_is_avif_editor_available() ) {
		...
		if( class_exists( '\Imagick' ) && ! $is_near_lossless ) {
			echo '<p><small class="note">'. esc_html__( 'Lossless compression uses PNG quality level.', 'still-be-image-quality-control' );
		...
		echo       '</tr>';
		echo       '<tr>';
		echo         '<th>'. esc_html__( 'Image Editor', 'still-be-image-quality-control' ). '</th>';
		echo         '<td class="sb-ti-info sb-ti-editor left">-</td>';
		echo       '</tr>';
		// @since 0.9.0
		if( class_exists( '\Imagick' ) || $is_lossless_options ) {
			echo   '<tr>';
			echo     '<th>'. esc_html__( 'Compression Mode', 'still-be-image-quality-control' ). '</th>';
			echo     '<td class="sb-ti-info sb-ti-mode left">-</td>';
			echo   '</tr>';
			echo   '<tr>';
			echo     '<th>'. esc_html__( 'Loss-Less Compression Level', 'still-be-image-quality-control' ). '</th>';
			echo     '<td class="sb-ti-info sb-ti-comp-level left">-</td>';
			echo   '</tr>';
		}
		echo       '<tr>';
		...
		echo       '</tr>';
		echo       '<tr>';
		echo         '<th>'. esc_html__( 'SSIM', 'still-be-image-quality-control' ). '</th>';
		echo         '<td class="sb-ti-info sb-ti-ssim left">-</td>';
		echo       '</tr>';
		echo       '<tr>';
		echo         '<th>'. esc_html__( 'SSIM in dB', 'still-be-image-quality-control' ). '</th>';
		echo         '<td class="sb-ti-info sb-ti-ssim-in-db left">-</td>';
		echo       '</tr>';
		echo       '<tr>';
		echo         '<th>'. esc_html__( 'SSIM Time', 'still-be-image-quality-control' ). '</th>';
		echo         '<td class="sb-ti-info sb-ti-ssim-time left">-</td>';
		echo       '</tr>';
		echo     '</table>';
		echo   '</div>';
		...
		echo       '<tr>';
		echo         '<th>'. esc_html__( 'Image Editor', 'still-be-image-quality-control' ). '</th>';
		echo         '<td class="sb-ti-info sb-ti-editor right">-</td>';
		echo       '</tr>';
		// @since 0.9.0
		if( class_exists( '\Imagick' ) || $is_lossless_options ) {
			echo   '<tr>';
		...
		echo       '<tr>';
		echo         '<th>'. esc_html__( 'SSIM', 'still-be-image-quality-control' ). '</th>';
		echo         '<td class="sb-ti-info sb-ti-ssim right">-</td>';
		echo       '</tr>';
		echo       '<tr>';
		echo         '<th>'. esc_html__( 'SSIM in dB', 'still-be-image-quality-control' ). '</th>';
		echo         '<td class="sb-ti-info sb-ti-ssim-in-db right">-</td>';
		echo       '</tr>';
		echo       '<tr>';
		echo         '<th>'. esc_html__( 'SSIM Time', 'still-be-image-quality-control' ). '</th>';
		echo         '<td class="sb-ti-info sb-ti-ssim-time right">-</td>';
		echo       '</tr>';
		echo     '</table>';
		echo   '</div>';
		...
		// Left ↔ Right SSIM (WebGPU / CPU). Markup is required by admin-test-image.js.
		echo '<div class="sb-compare-ssim-wrap">';
		echo   '<p id="sb_compare_ssim" class="sb-compare-ssim">';
		echo     '<span class="sb-compare-ssim-label">'. esc_html__( 'Left ↔ Right SSIM', 'still-be-image-quality-control' ). '</span>';
		echo     '<span class="sb-compare-ssim-value">-</span>';
		echo   '</p>';
		echo   '<div class="sb-compare-ssim-mode">';
		echo     '<label>';
		echo       '<input type="radio" name="sb_compare_ssim_mode" value="dense">';
		echo       esc_html__( 'Exact (all pixels)', 'still-be-image-quality-control' );
		echo     '</label>';
		echo     '<label>';
		echo       '<input type="radio" name="sb_compare_ssim_mode" value="sparse" checked>';
		echo       esc_html__( 'Sampled (faster)', 'still-be-image-quality-control' );
		echo     '</label>';
		echo   '</div>';
		echo '</div>';
