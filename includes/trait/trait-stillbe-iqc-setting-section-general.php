...
trait Setting_Section_General {
	...
	public function render_quality_level_table( $args ) {
		...
		// Render HTML
		echo '<p>'. esc_html__( 'Blanks and 0 or less are not set.', 'still-be-image-quality-control' ). '</p>';
		echo '<p>'. esc_html__( 'Enter a number from 1-100 for JPEG / WebP, 1-99 for AVIF, and 1-9 for PNG.', 'still-be-image-quality-control' ). '</p>';
		echo '<p>';
		...
		echo '</p>';
		echo '<p>';
		echo   esc_html__( 'For AVIF, comparable quality can be achieved at lower values than JPEG / WebP. About 50-70 is a good starting point.', 'still-be-image-quality-control' ). '<br>';
		echo   esc_html__( 'AVIF quality is limited to 99 because 100 is reserved for lossless encoding.', 'still-be-image-quality-control' );
		echo '</p>';
		echo '<p>';
		echo   esc_html__( 'For PNG, 9 is the best choice because it has the highest compression ratio.', 'still-be-image-quality-control' ). ' ';
		echo   esc_html__( 'Try changing the level to 5-6 only if the server load for PNG resizing is high.', 'still-be-image-quality-control' );
		if( $this->is_lossless_options && ! $this->is_near_lossless ) {
			echo '<br>';
			echo esc_html__( 'Especially, large PNG images take a long time to compress with lossless encoding.', 'still-be-image-quality-control' ). ' ';
			echo esc_html__( '(Lossless compression uses PNG quality level)', 'still-be-image-quality-control' );
		}
		echo '</p>';
		...
		echo     '<label>';
		echo       '<input type="radio" name="'. esc_attr( self::SETTING_NAME. '[toggle][enable-auto-optimize]' ). '" value="true"'.  ( $is_auto_optimized ? ' checked' : '' ). '>';
		echo       '<span>'. esc_html__( 'Enable', 'still-be-image-quality-control' ). '</span>';
		echo     '</label>';
		echo     '<label>';
		echo       '<input type="radio" name="'. esc_attr( self::SETTING_NAME. '[toggle][enable-auto-optimize]' ). '" value="false"'. ( $is_auto_optimized ? '' : ' checked' ). '>';
		echo       '<span>'. esc_html__( 'Disable', 'still-be-image-quality-control' ). '</span>';
		echo     '</label>';
		echo   '</div>';
		echo   '<p>';
		echo     esc_html__( '* Immediately after uploading, the compression ratio is applied as shown in the table below. Optimization will be performed step by step by WP-Cron.', 'still-be-image-quality-control' );
		echo   '</p>';
		echo   '<div class="field-line" style="margin-top: 0.6em;">';
		echo     '<span style="margin-right: 1em;">'. esc_html__( 'Optimization Target', 'still-be-image-quality-control' ). ':</span>';
		echo     '<label>';
		echo       '<input type="radio" name="'. esc_attr( self::SETTING_NAME. '[auto-optimize-target]' ). '" value="efficiency"'. ( 'efficiency' === $ao_target ? ' checked' : '' ). '>';
		echo       '<span>'. esc_html__( 'Efficiency (smaller file size)', 'still-be-image-quality-control' ). '</span>';
		echo     '</label>';
		echo     '<label>';
		echo       '<input type="radio" name="'. esc_attr( self::SETTING_NAME. '[auto-optimize-target]' ). '" value="balance"'.    ( 'balance'    === $ao_target ? ' checked' : '' ). '>';
		echo       '<span>'. esc_html__( 'Balance', 'still-be-image-quality-control' ). '</span>';
		echo     '</label>';
		echo     '<label>';
		echo       '<input type="radio" name="'. esc_attr( self::SETTING_NAME. '[auto-optimize-target]' ). '" value="quality"'.    ( 'quality'    === $ao_target ? ' checked' : '' ). '>';
		echo       '<span>'. esc_html__( 'Quality (higher fidelity)', 'still-be-image-quality-control' ). '</span>';
		echo     '</label>';
		echo   '</div>';
		echo '</div>';
		...
		foreach( $sizes as $name => $size ) {
			...
			$q_avif = empty( $qualities[( $name. '_avif' )] ) ? '' : $qualities[( $name. '_avif' )];
			...
			if( in_array( $name, $add_size_names ) ){
				...
				echo   '<td><input type="number" name="'.   esc_attr( self::SETTING_NAME. '[image-size]['. $_i. '][jpeg]' ).   '" value="'. esc_attr( $q_jpeg ). '" min="1" max="100"></td>';
				echo   '<td><input type="number" name="'.   esc_attr( self::SETTING_NAME. '[image-size]['. $_i. '][png]'  ).   '" value="'. esc_attr( $q_png  ). '" min="1" max="9"></td>';
				echo   '<td><input type="number" name="'.   esc_attr( self::SETTING_NAME. '[image-size]['. $_i. '][webp]' ).   '" value="'. esc_attr( $q_webp ). '" min="1" max="100"></td>';
				echo   '<td><input type="number" name="'.   esc_attr( self::SETTING_NAME. '[image-size]['. $_i. '][avif]' ).   '" value="'. esc_attr( $q_avif ). '" min="1" max="99"></td>';
				echo '</tr>';
				$_i++;
			} else {
				echo '<tr class="'. esc_attr( $_class ). '">';
				...
				echo   '<td><input type="number" name="'. esc_attr( self::SETTING_NAME. '[quality]['. $name. '][jpeg]' ). '" value="'. esc_attr( $q_jpeg ). '" min="1" max="100"></td>';
				echo   '<td><input type="number" name="'. esc_attr( self::SETTING_NAME. '[quality]['. $name. '][png]'  ). '" value="'. esc_attr( $q_png  ). '" min="1" max="9"></td>';
				echo   '<td><input type="number" name="'. esc_attr( self::SETTING_NAME. '[quality]['. $name. '][webp]' ). '" value="'. esc_attr( $q_webp ). '" min="1" max="100"></td>';
				echo   '<td><input type="number" name="'. esc_attr( self::SETTING_NAME. '[quality]['. $name. '][avif]' ). '" value="'. esc_attr( $q_avif ). '" min="1" max="99"></td>';
				echo '</tr>';
			}
		}
		echo     '</tbody>';
		...
		echo       '<th><input type="number" name="'.   esc_attr( self::SETTING_NAME. '[image-size][{{i}}][jpeg]' ).   '" min="1" max="100"></th>';
		echo       '<th><input type="number" name="'.   esc_attr( self::SETTING_NAME. '[image-size][{{i}}][png]' ).    '" min="1" max="9"></th>';
		echo       '<th><input type="number" name="'.   esc_attr( self::SETTING_NAME. '[image-size][{{i}}][webp]' ).   '" min="1" max="100"></th>';
		echo       '<th><input type="number" name="'.   esc_attr( self::SETTING_NAME. '[image-size][{{i}}][avif]' ).   '" min="1" max="99"></th>';
		echo     '</tr>';
		echo   '</template>';
		echo '</div>';
	...
	public function render_quality_level_default( $args ) {
		...
		$def_avif = empty( $qualities['default_avif'] ) ? '' : $qualities['default_avif'];
		...
		echo     '<tbody>';
		echo       '<tr>';
		echo         '<td><input type="number" name="'. esc_attr( self::SETTING_NAME. '[quality][default][jpeg]' ). '" value="'. esc_attr( $def_jpeg ). '" min="1" max="100"></td>';
		echo         '<td><input type="number" name="'. esc_attr( self::SETTING_NAME. '[quality][default][png]'  ). '" value="'. esc_attr( $def_png  ). '" min="1" max="9"></td>';
		echo         '<td><input type="number" name="'. esc_attr( self::SETTING_NAME. '[quality][default][webp]' ). '" value="'. esc_attr( $def_webp ). '" min="1" max="100"></td>';
		echo         '<td><input type="number" name="'. esc_attr( self::SETTING_NAME. '[quality][default][avif]' ). '" value="'. esc_attr( $def_avif ). '" min="1" max="99"></td>';
		echo       '</tr>';
		echo     '</tbody>';
		echo   '</table>';
		echo '</div>';
	...
	public function render_quality_level_original( $args ) {
		...
		if( ! is_array( $original_webp ) ) {
			...
			$original_webp = self::chk_num_type( $original_webp, 'webp' ) ?: $_defaults['original_webp'];
			$original_webp = array(
				array(
					'lossy'    => $original_webp,
					'lossless' => 9,
