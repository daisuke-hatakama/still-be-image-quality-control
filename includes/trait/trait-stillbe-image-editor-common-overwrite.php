...
trait Image_Editor_Common_Overwrite {
	...
	public function make_subsize( $size_data ) {
		...
		if( ! is_wp_error( $meta ) && stillbe_iqc_should_generate_delivery_webp( $this->mime_type, $this->file ) ) {
			if( apply_filters( 'stillbe_image_quality_control_enable_save_webp_sync', STILLBE_IQ_ENABLE_SAVE_WEBP_SYNC, $meta ) ) {
				// Save WebP Synchronously
				$basedir   = dirname( $this->file );
				$webp_name = stillbe_iqc_get_filtered_delivery_webp_path( path_join( $basedir, $meta['file'] ) );
				if( $webp_name ) {
					...
					if( ! is_wp_error( $webp_data ) && ! empty( $webp_data['size'] ) ) {
						...
						stillbe_iqc_merge_encode_method_meta( $meta['sb-iqc'], $webp_data, 'webp' );
					}
				}
			} else {
				// Save WebP Asynchronously
				// Attachment ID が確定した後に WP-Cron を設定するのでここでは何もしない
			}
		}

		if( ! is_wp_error( $meta ) && stillbe_iqc_should_generate_delivery_avif( $this->mime_type, $this->file ) ) {
			if( apply_filters( 'stillbe_image_quality_control_enable_save_avif_sync', STILLBE_IQ_ENABLE_SAVE_AVIF_SYNC, $meta ) ) {
				// Save AVIF Synchronously
				$basedir   = dirname( $this->file );
				$avif_name = stillbe_iqc_get_filtered_delivery_avif_path( path_join( $basedir, $meta['file'] ) );
				if( $avif_name ) {
					$avif_data = $this->make_avif( $avif_name, $size_data );
					if( ! is_wp_error( $avif_data ) && ! empty( $avif_data['size'] ) ) {
						$meta['sb-iqc']['avif-file']    = $avif_data['file'];
						$meta['sb-iqc']['avif-quality'] = $avif_data['q'];
						stillbe_iqc_merge_encode_method_meta( $meta['sb-iqc'], $avif_data, 'avif' );
					}
				}
			}
		}
	...
	public function generate_filename( $suffix = null, $dest_path = null, $extension = null ) {
		...
		$sb_generated_name = $this->generate_safe_filename( $wp_generated_name, $suffix );

		// Return after Filtering
		return apply_filters( "stillbe_uploaded_image_{$this->mime_type}_name", $sb_generated_name );
